...
const settings = {
    ...
    plugins: defaultSettings.plugins.concat(plugin("test-plugin", (params) => {
...
const ContextMockWrapper = ({ children }) => _jsx(TranslationProvider, { value: settings, children: children });
...
describe("using function in template settings", () => {
    ...
    test("Should not break when template is repeated many times with different values", () => {
        ...
        expect(t("name", {
            name: "Jack",
        }, "zh")).toBe("Jack, Jack and Jack and Jack");
    });
